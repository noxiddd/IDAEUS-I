...
void sim_setup(int widthh,int heighth)
{
 //size (650, 500, P3D); //P3D for rendering 3D object
  e1 = new earth(10, 0, 0, 13, 5, 0, widthh, heighth); //reset the value for earth class
}


void sim_draw(int widthh,int heighth)
{
  ...
  noFill(); // 
  stroke(150);
  ellipse(widthh/2, heighth/2, radius*2, radius*2);
  noStroke();
  /* trace of orbit animation*/
  ...
  pushMatrix(); 
  translate(x+(widthh/2), y+(heighth/2));//puts the center 
  e1.update();
  popMatrix();
  /* draw orbit animation with earth class*/
...
class earth {
  ...
  float earthSize, moonXpos, moonYpos, moonDist, moonSize, degree,heighthh,widthhh;
  earth (float sizeE, float x, float y, float d, float sizeM, float d1, int heighth, int widthh) {
  ...
  void update() {
   ...
   // pushMatrix();
//    translate(widthhh/2, heighthh/2); //to draw object on center of canvas
 ...
 //   translate(-widthhh/2, -heighthh/2); //reset canvas matrix
 //   popMatrix();
  }
}
